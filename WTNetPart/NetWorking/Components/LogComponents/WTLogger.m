...
@property (nonatomic, strong, readwrite) WTLoggerConfiguration *configParams;
...
+ (void)logDebugInfoWithRequest:(NSURLRequest *)request apiName:(NSString *)apiName service:(WTService *)service requestParams:(id)requestParams httpMethod:(NSString *)httpMethod
{
    ...
    [logString appendFormat:@"API Name:\t\t%@\n", [apiName WT_defaultValue:@"N/A"]];
    [logString appendFormat:@"Method:\t\t\t%@\n", [httpMethod WT_defaultValue:@"N/A"]];
    [logString appendFormat:@"Version:\t\t%@\n", [service.apiVersion WT_defaultValue:@"N/A"]];
    [logString appendFormat:@"Service:\t\t%@\n", [service class]];
    [logString appendFormat:@"Status:\t\t\t%@\n", isOnline ? @"online" : @"offline"];
    [logString appendFormat:@"Public Key:\t\t%@\n", [service.publicKey WT_defaultValue:@"N/A"]];
    [logString appendFormat:@"Private Key:\t%@\n", [service.privateKey WT_defaultValue:@"N/A"]];
...
+ (void)logDebugInfoWithCachedResponse:(WTURLResponse *)response methodName:(NSString *)methodName serviceIdentifier:(WTService *)service
{
    ...
    [logString appendFormat:@"API Name:\t\t%@\n", [methodName WT_defaultValue:@"N/A"]];
    [logString appendFormat:@"Version:\t\t%@\n", [service.apiVersion WT_defaultValue:@"N/A"]];
    [logString appendFormat:@"Service:\t\t%@\n", [service class]];
    [logString appendFormat:@"Public Key:\t\t%@\n", [service.publicKey WT_defaultValue:@"N/A"]];
    [logString appendFormat:@"Private Key:\t%@\n", [service.privateKey WT_defaultValue:@"N/A"]];
    ...
    [logString appendFormat:@"ModesData:\n\t%@\n\n", response.modelsData];
    [logString appendFormat:@"\n\n==============================================================\n=                        Response End                        =\n==============================================================\n\n\n\n"];
    NSLog(@"%@", logString);
#endif
}
+ (instancetype)sharedInstance
{
    ...
    static WTLogger *sharedInstance;
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.configParams = [[WTLoggerConfiguration alloc] init];
    }
    return self;
}
- (void)logWithActionCode:(NSString *)actionCode params:(NSDictionary *)params
{
   ...
   // [actionDict addEntriesFromDictionary:[CTCommonParamsGenerator commonParamsDictionaryForLog]];
    NSDictionary *logJsonDict = @{self.configParams.sendActionKey:[@[actionDict] WT_jsonString]};
  //  [[WTApiProxy sharedInstance] callPOSTWithParams:logJsonDict serviceIdentifier:self.configParams.serviceType relativeURL:self.configParams.sendActionMethod success:nil fail:nil];
    [[WTApiProxy sharedInstance] callPOSTWithParams:logJsonDict serviceIdentifier:NSClassFromString(self.configParams.serviceType) relativeURL:self.configParams.sendActionMethod success:nil fail:nil];
}
