...
@property (nonatomic, strong)WTCompleteBlock completeBlock;
@end
...
- (NSInteger)loadDataWithParams:(NSDictionary *)params withCompleteBlock:(WTCompleteBlock)block {
    if (block) {
    ...
    if (self.validator ==nil ||[self.validator manager:self isCorrectWithCallBackData:response.content]) {
        ...
        if ([self beforePerformSuccessWithResponse:response]) {
            ...
            if (self.completeBlock) {
                ...
                self.completeBlock(response,WTAPIManagerErrorTypeSuccess);
                return;
            }
        ...
        if (self.completeBlock) {
            ...
            self.completeBlock(response,WTAPIManagerErrorTypeNoContent);
            return;
        }
...
//    } else {
        ...
        if ([self beforePerformFailWithResponse:response]) {
            ...
            if (self.completeBlock) {
                ...
                self.completeBlock(response,errorType);
                return;
            }
...
- (BOOL)loadDataFromNative
{
    ...
    NSException *exception = [[NSException alloc]initWithName:@"调用父类未实现的方法" reason:@"需要在子类中实现loadDataFromNative数据存入fetchedRawData中" userInfo:nil];
